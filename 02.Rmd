...
To accomplish the identification of the type of garment, stitch pattern(s), guage, and suggested yarn, Siliki uses image recognition along with machine learning.  
...
Once Siliki identifies the type of garment, the stitch patterns, the guage, and receives the measurements from the user, a pattern can be built following rather simple proportional calculations for stitch counts and shaping. Siliki will synthesize all of the output to create a beautifully formatted custom pattern. The pattern will contain information on needle sizes and recommended yarns based on suitability and popularity on Ravelry. Additionally, Siliki will output patterns that are similar to what the user is trying to create. If Ravelry contains a pattern for the exact item the user inputs, a link to that pattern will be returned, with the option to purchase it.
